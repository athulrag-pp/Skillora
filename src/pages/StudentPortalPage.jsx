  ...
  const modules = [
    {
      ...
      status: 'Completed',
      progress: 100,
      badgeColor: theme === 'dark' ? 'bg-emerald-950 text-emerald-300 border-emerald-500/40' : 'bg-emerald-100 text-emerald-800 border-emerald-300',
      topics: [
    ...
    },
    {
      ...
      status: 'In Progress',
      progress: 75,
      badgeColor: theme === 'dark' ? 'bg-indigo-950 text-indigo-300 border-indigo-500/40' : 'bg-indigo-100 text-indigo-800 border-indigo-300',
      topics: [
    ...
    },
    {
      ...
      status: 'Next Up',
      progress: 0,
      badgeColor: theme === 'dark' ? 'bg-amber-950 text-amber-300 border-amber-500/40' : 'bg-amber-100 text-amber-800 border-amber-300',
      topics: [
    ...
    },
    {
      ...
      status: 'Locked',
      progress: 0,
      badgeColor: theme === 'dark' ? 'bg-gray-800 text-gray-400 border-gray-700' : 'bg-slate-100 text-slate-600 border-slate-300',
      topics: [
  ...
  return (
    <div className="p-4 lg:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className={`p-6 rounded-3xl border transition-all ${
        theme === 'dark'
          ? 'glass-panel border-indigo-500/40 bg-gradient-to-r from-indigo-950/80 via-purple-950/60 to-slate-950/80 text-white'
          : 'bg-gradient-to-r from-indigo-900 via-indigo-800 to-purple-900 text-white shadow-xl border-indigo-700'
      }`}>
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="flex items-center space-x-4">
            <div className="relative">
              <img 
                src={student.avatar} 
                alt={student.name} 
                className="w-16 h-16 rounded-2xl object-cover border-2 border-indigo-400 shadow-2xl" 
              />
              <span className="absolute -bottom-1 -right-1 w-4 h-4 bg-emerald-500 border-2 border-indigo-950 rounded-full" title="Active Online" />
            </div>
            <div>
              <div className="flex items-center space-x-2.5 flex-wrap">
                <h1 className="text-2xl font-black tracking-tight">{student.name}</h1>
                <span className="text-xs font-bold text-cyan-300 bg-cyan-950/80 px-3 py-1 rounded-full border border-cyan-500/40 flex items-center space-x-1">
                  <ShieldCheck className="w-3.5 h-3.5 text-cyan-400" />
                  <span>My Student Profile ({student.id})</span>
                </span>
              </div>
              <p className="text-xs text-indigo-200 mt-1 font-medium">
                {student.courseName} • Enrolled Batch: <strong className="text-white font-bold">{student.batchName}</strong> • Mentor: <span className="text-purple-300 font-semibold">{student.trainerName}</span>
              </p>
          ...
          <div className="flex items-center space-x-3 self-stretch md:self-auto justify-between md:justify-end border-t md:border-t-0 pt-4 md:pt-0 border-indigo-700/50">
            <div className="bg-black/40 backdrop-blur-md p-3.5 rounded-2xl border border-white/10 text-center min-w-[100px]">
              <span className="text-[10px] uppercase font-extrabold text-indigo-300 block">Overall Score</span>
              <span className="text-2xl font-black text-white">{student.overallScore}%</span>
            </div>
            <div className="bg-black/40 backdrop-blur-md p-3.5 rounded-2xl border border-white/10 text-center min-w-[100px]">
              <span className="text-[10px] uppercase font-extrabold text-indigo-300 block">Grade</span>
              <span className="text-2xl font-black text-emerald-400">Grade {student.grade}</span>
            </div>
          </div>
        </div>

        {/* Tab Navigation Header */}
        <div className="mt-6 pt-4 border-t border-white/10 flex items-center space-x-2 overflow-x-auto">
          <button
            onClick={() => { setActiveTab('dashboard'); navigateTo('student_portal'); }}
            className={`flex items-center space-x-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${
              activeTab === 'dashboard'
                ? 'bg-white text-indigo-950 shadow-lg glow-primary font-extrabold'
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className={`p-4 rounded-2xl border transition-all ${
              ...
              <div className="text-3xl font-black text-emerald-500 mt-1">{student.academic}%</div>
            ...
            <div className={`p-4 rounded-2xl border transition-all ${
              ...
              <div className="text-3xl font-black text-amber-500 mt-1">{student.attendance}%</div>
            ...
            <div className={`p-4 rounded-2xl border transition-all ${
              ...
              <div className="text-3xl font-black text-purple-500 mt-1">{student.assignment}%</div>
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full inline-block mt-2 ${
            ...
            <div className={`p-4 rounded-2xl border transition-all ${
              ...
              <div className="text-3xl font-black text-cyan-500 mt-1">{student.progress}%</div>
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full inline-block mt-2 ${
          ...
          <div className={`p-5 rounded-2xl border transition-all ${
            ...
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center space-x-2 text-indigo-400 text-xs font-extrabold uppercase tracking-wider">
                <Sparkles className="w-4 h-4 text-amber-400 animate-spin-slow" />
                <span>AI Personal Mentor Insight for {student.name}</span>
              </div>
              <span className="text-[10px] bg-indigo-600/30 text-indigo-300 px-2 py-0.5 rounded border border-indigo-500/30 font-mono">
                Realtime Analysis
              </span>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className={`lg:col-span-2 p-5 rounded-2xl border ${
              ...
              <div className="mt-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 rounded-xl border border-purple-500/20 bg-purple-950/20">
                <div>
                  <div className="text-xs font-bold text-purple-300 uppercase tracking-wider">Batch: {student.batchName}</div>
                  ...
                  <p className={`text-xs mt-1 ${theme === 'dark' ? 'text-gray-400' : 'text-slate-600'}`}>
                    Trainer: <strong className="text-white">{student.trainerName}</strong> • Today at 6:00 PM IST (60 mins)
                  </p>
                </div>
                <button
              ...
              <div className="mt-6">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-xs font-bold uppercase text-gray-400">Current Course Roadmap</h3>
                  <button 
                    onClick={() => { setActiveTab('curriculum'); navigateTo('student_curriculum'); }}
                ...
                <div className="space-y-2">
                  {modules.slice(0, 3).map(mod => (
                    <div key={mod.id} className={`p-3 rounded-xl border flex items-center justify-between ${
                      theme === 'dark' ? 'bg-gray-950/50 border-gray-800' : 'bg-slate-50 border-slate-200'
                    }`}>
                      <div className="flex items-center space-x-3">
                        <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold ${
                          mod.status === 'Completed' ? 'bg-emerald-500/20 text-emerald-400' :
                          mod.status === 'In Progress' ? 'bg-indigo-500/20 text-indigo-400' : 'bg-amber-500/20 text-amber-400'
                        }`}>
            ...
            <div className="space-y-6">
              ...
              <div className={`p-5 rounded-2xl border ${
                ...
                <div className="space-y-2 text-xs">
                  ...
                  <div className="flex justify-between py-1 border-b border-gray-800/40">
                    ...
                    <strong className="text-emerald-400 font-bold">₹85,000 (100%)</strong>
                  </div>
                  <div className="flex justify-between py-1 border-b border-gray-800/40">
                    <span className={theme === 'dark' ? 'text-gray-400' : 'text-slate-600'}>Payment Status:</span>
                    <span className="text-[10px] font-extrabold bg-emerald-950 text-emerald-300 border border-emerald-500/30 px-2 py-0.5 rounded">
                      FULLY PAID
                    </span>
                  </div>
                </div>
              ...
              <div className={`p-5 rounded-2xl border ${
                ...
                <h3 className="text-xs font-bold uppercase text-gray-400 mb-3">Individual Student Study Hub</h3>
                <div className="space-y-2">
                  <a href="#lab" onClick={(e) => { e.preventDefault(); showToast('Opening PyTorch Jupyter Lab Sandbox...', 'info'); }} className={`p-2.5 rounded-xl border flex items-center justify-between text-xs transition-colors ${
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span>PyTorch Jupyter Notebooks</span>
                    </div>
                  ...
                  <a href="#github" onClick={(e) => { e.preventDefault(); showToast('Opening Class GitHub Repository...', 'info'); }} className={`p-2.5 rounded-xl border flex items-center justify-between text-xs transition-colors ${
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span>Class Code & Project Repo</span>
                    </div>
                    <ExternalLink className="w-3 h-3 text-gray-400" />
                  </a>
                </div>
          ...
          <div className={`p-5 rounded-2xl border ${
            ...
            <div className="flex items-center justify-between mb-4 pb-3 border-b border-gray-800/40">
              ...
              <div className="text-right">
                <span className="text-xs font-bold text-indigo-400 block">Overall Course Progress</span>
                <span className="text-xl font-black text-indigo-500">{student.progress}%</span>
              </div>
            </div>
                ...
                return (
                  <div key={mod.id} className={`rounded-2xl border transition-all overflow-hidden ${
                    theme === 'dark'
                      ...
                      : (isExpanded ? 'bg-indigo-50/50 border-indigo-300' : 'bg-slate-50 border-slate-200')
                  }`}>
                    <button
                    ...
                    >
                      <div className="flex items-center space-x-4">
                        <div className={`w-10 h-10 rounded-xl font-black text-sm flex items-center justify-center ${
                          mod.status === 'Completed' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' :
                          mod.status === 'In Progress' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-gray-800 text-gray-400 border border-gray-700'
                        }`}>
                          {mod.number.replace('Module ', '')}
                        </div>
                        <div>
                          <div className="flex items-center space-x-2">
                            <span className="text-xs font-extrabold text-indigo-400 uppercase tracking-wider">{mod.number}</span>
                            <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${mod.badgeColor}`}>
                              {mod.status}
                            </span>
                          </div>
                      ...
                      <div className="flex items-center space-x-3">
                        <div className="hidden sm:block text-right">
                          <span className="text-[10px] text-gray-400 font-bold uppercase block">Progress</span>
                          <span className="text-xs font-black">{mod.progress}%</span>
                        </div>
                        <ChevronRight className={`w-4 h-4 transition-transform ${isExpanded ? 'rotate-90 text-indigo-400' : 'text-gray-500'}`} />
                      </div>
                    </button>
                    ...
                    {isExpanded && (
                      <div className={`p-4 border-t space-y-2 ${
                        ...
                        <div className="text-[10px] font-extrabold uppercase text-gray-400 tracking-wider mb-2">
                        ...
                        {mod.topics.map((topic, idx) => (
                          <div key={idx} className={`p-3 rounded-xl border flex items-center justify-between text-xs ${
                            theme === 'dark' ? 'bg-gray-900/60 border-gray-800 text-gray-200' : 'bg-slate-50 border-slate-200 text-slate-800'
                          }`}>
                            <div className="flex items-center space-x-3">
                              {topic.done ? (
                            ...
                            <button 
                              ...
                              className="px-3 py-1 rounded-lg bg-indigo-600/20 hover:bg-indigo-600/30 text-indigo-400 text-[11px] font-bold border border-indigo-500/30 transition-colors"
                            >
      ...
      {activeTab === 'assignments' && (
        <div className="space-y-6">
          <div className={`p-5 rounded-2xl border ${
            ...
            <h2 className="text-sm font-black uppercase tracking-wider mb-4 text-indigo-400">
            ...
            <div className="space-y-3">
              {assignments.map(asn => (
                <div key={asn.id} className={`p-4 rounded-xl border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 ${
                  theme === 'dark' ? 'bg-gray-950/60 border-gray-800 text-gray-200' : 'bg-slate-50 border-slate-200 text-slate-800'
                }`}>
                  <div>
                    <div className="flex items-center space-x-2">
                      <span className="text-[10px] font-mono font-bold bg-indigo-950 text-indigo-300 border border-indigo-500/30 px-2 py-0.5 rounded">
                        {asn.id}
                      </span>
                      <span className="text-xs font-bold text-gray-400">{asn.module}</span>
                    </div>
                  ...
                  <div className="flex items-center space-x-4 self-end sm:self-auto">
                    <div className="text-right">
                      ...
                      <span className="text-xs font-black text-emerald-400">{asn.score}</span>
                    </div>
                    <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full border ${
                      asn.status === 'Graded' ? 'bg-emerald-950 text-emerald-300 border-emerald-500/30' :
                      asn.status === 'Submitted' ? 'bg-indigo-950 text-indigo-300 border-indigo-500/30' : 'bg-amber-950 text-amber-300 border-amber-500/30'
                    }`}>
                      {asn.status}
                    </span>
                  </div>
