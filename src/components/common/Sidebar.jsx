...
export const Sidebar = () => {
  const { currentRole, activePage, navigateTo, changeRole } = useSkillora();
  ...
  return (
    <aside className="w-64 glass-panel border-r border-gray-800/80 flex flex-col h-screen sticky top-0 z-40 select-none">
      {/* Brand Logo & Tagline Header */}
      <div className="p-4 border-b border-gray-800/80 flex flex-col">
        <div className="flex items-center space-x-2.5">
          ...
          <div>
            <h1 className="text-xl font-black bg-gradient-to-r from-white via-indigo-200 to-indigo-400 bg-clip-text text-transparent tracking-wider">
              SKILLORA
            </h1>
        ...
        <div className="mt-3 bg-indigo-950/40 border border-indigo-500/20 rounded-lg p-2 flex items-center justify-between">
          <span className="text-[11px] font-medium text-indigo-300">Active Role:</span>
          <span className="text-[11px] font-bold text-white bg-indigo-600/40 px-2 py-0.5 rounded border border-indigo-500/40">
            {currentRole}
          </span>
        </div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        <div className="px-3 pb-2 text-[10px] font-extrabold uppercase text-gray-500 tracking-wider">
          Main Navigation
        </div>
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <div className="flex items-center space-x-3">
                <Icon className={`w-4 h-4 transition-transform group-hover:scale-110 ${isActive ? 'text-white' : 'text-gray-400 group-hover:text-indigo-400'}`} />
      ...
      <div className="p-3 border-t border-gray-800/80 bg-gray-950/60">
        <button
          ...
          className="w-full flex items-center justify-center space-x-2 bg-gray-900 hover:bg-gray-800 text-gray-300 hover:text-white text-xs font-medium py-2 rounded-lg border border-gray-800 transition-colors"
        >
